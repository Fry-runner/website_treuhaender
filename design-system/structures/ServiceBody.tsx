...
export const ServiceBody: React.FC<{ title: string; summary: string; bullets: string[]; body?: string }> = ({ title, summary, bullets, body }) => (
      ...
      <div style={{ display: "grid", gridTemplateColumns: "minmax(0,1.6fr) minmax(0,1fr)", gap: "3rem", alignItems: "start" }}>
        <div style={{ display: "flex", flexDirection: "column", gap: "1.2rem" }}>
          ...
          <p style={{ fontFamily: "var(--ds-font-body)", lineHeight: 1.7, color: "var(--ds-text-muted)", margin: 0, maxWidth: "60ch" }}>
            {body ?? `Wir übernehmen ${title.toLowerCase()} vollständig und termingerecht – digital, transparent und persönlich betreut. So behalten Sie jederzeit den Überblick über Ihre Zahlen.`}
          </p>
        </div>
