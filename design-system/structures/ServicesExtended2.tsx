...
const priceS: React.CSSProperties = { fontFamily: "var(--ds-font-mono)", fontSize: "0.78rem", color: "var(--ds-primary)", fontWeight: 600 };
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px,1fr))", gap: "1.2rem" }}>
        {content.items.map((s, i) => (
          <article key={i} role={onPick ? "button" : undefined} tabIndex={onPick ? 0 : undefined} onClick={onPick ? () => onPick(s.title) : undefined} onKeyDown={onPick ? (e) => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); onPick(s.title); } } : undefined} style={{ background: "var(--ds-surface)", border: "1px solid var(--ds-border)", borderRadius: "var(--ds-radius)", boxShadow: "var(--ds-shadow-card)", padding: "1.8rem", display: "flex", flexDirection: "column", alignItems: "center", textAlign: "center", gap: "0.7rem", cursor: onPick ? "pointer" : "default" }}>
            <div style={{ width: "3rem", height: "3rem", borderRadius: "9999px", background: "var(--ds-primary-soft)", color: "var(--ds-primary)", display: "flex", alignItems: "center", justifyContent: "center" }}><Icon name="check" size={18} /></div>
...
export const ServicesListArrow: React.FC<Props> = ({ content, more, onPick }) => (
  <section style={sectionBase}>
    <Container style={{ maxWidth: "min(var(--ds-container), 880px)" }}>
      ...
      <div>
        {content.items.map((s, i) => (
          <div key={i} role={onPick ? "button" : undefined} tabIndex={onPick ? 0 : undefined} onClick={onPick ? () => onPick(s.title) : undefined} onKeyDown={onPick ? (e) => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); onPick(s.title); } } : undefined} style={{ display: "grid", gridTemplateColumns: "1fr auto", gap: "1.4rem", alignItems: "center", padding: "1.3rem 0", borderTop: "1px solid var(--ds-border)", cursor: onPick ? "pointer" : "default" }}>
            <div style={{ display: "flex", flexDirection: "column", gap: "0.3rem" }}><h3 style={titleS}>{s.title}</h3><p style={bodyS}>{s.summary}</p></div>
            <span aria-hidden style={{ color: "var(--ds-primary)", display: "inline-flex" }}><Icon name="arrowRight" size={20} /></span>
          </div>
        ))}
      </div>
    </Container>
    ...
    <section style={sectionBase}>
      <Container>
        ...
        <div role="tabpanel" style={{ background: "var(--ds-surface)", border: "1px solid var(--ds-border)", borderRadius: "var(--ds-radius)", padding: "2rem", display: "flex", flexDirection: "column", gap: "0.9rem", maxWidth: "640px", marginInline: "auto", textAlign: "center" }}>
          ...
          {onPick && <button onClick={() => onPick(sel.title)} style={{ alignSelf: "center", background: "none", border: "none", cursor: "pointer", ...priceS, color: "var(--ds-text-muted)" }}>Mehr erfahren <Icon name="arrowRight" size={13} style={{ verticalAlign: "-0.1em" }} /></button>}
        </div>
      </Container>
    </section>
  );
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(240px,1fr))", gap: "1.2rem" }}>
        {content.items.map((s, i) => (
          <article key={i} role={onPick ? "button" : undefined} tabIndex={onPick ? 0 : undefined} onClick={onPick ? () => onPick(s.title) : undefined} onKeyDown={onPick ? (e) => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); onPick(s.title); } } : undefined} style={{ background: "var(--ds-surface)", border: "1px solid var(--ds-border)", borderRadius: "var(--ds-radius)", overflow: "hidden", display: "flex", flexDirection: "column", cursor: onPick ? "pointer" : "default" }}>
            <div aria-hidden style={{ height: "11rem", ...(s.image ? { backgroundImage: `url("${s.image}")`, backgroundSize: "cover", backgroundPosition: "center" } : { backgroundImage: "linear-gradient(135deg, var(--ds-primary-soft), var(--ds-surface))" }) }} />
    ...
    <section style={sectionBase}>
      <Container>
        ...
        <div style={{ display: "grid", gridTemplateColumns: `repeat(${cols}, minmax(0,1fr))`, gap: "1.2rem" }}>
          {content.items.map((s, i) => (
            <article key={i} role={onPick ? "button" : undefined} tabIndex={onPick ? 0 : undefined} onClick={onPick ? () => onPick(s.title) : undefined} onKeyDown={onPick ? (e) => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); onPick(s.title); } } : undefined} style={{ border: "1px solid var(--ds-border)", borderRadius: "var(--ds-radius)", padding: "1.8rem", display: "flex", flexDirection: "column", gap: "0.6rem", cursor: onPick ? "pointer" : "default" }}>
              <h3 style={titleS}>{s.title}</h3>
              <p style={bodyS}>{s.summary}</p>
            </article>
          ))}
