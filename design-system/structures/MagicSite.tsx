    ...
    <section style={{ background: "var(--ds-bg)", paddingBlock: "var(--ds-section-y)", borderBottom: "1px solid var(--ds-border)" }}>
      <Container>
        <div style={{ display: "grid", gridTemplateColumns: "minmax(0,1.1fr) minmax(0,0.9fr)", gap: "3rem", alignItems: "center" }}>
          <div style={{ display: "flex", flexDirection: "column", gap: "1.85rem", maxWidth: "56ch" }}>
          ...
          <div style={{
            ...
            {!hasImg && (
              <div style={{ padding: "1.8rem", display: "flex", flexDirection: "column", gap: "1.15rem", height: "100%", justifyContent: "center" }}>
...
export const ServicesCardDecorator: React.FC<{ content: ServicesContent }> = ({ content }) => (
  <section style={{ background: "var(--ds-bg)", paddingBlock: "var(--ds-section-y)", borderBottom: "1px solid var(--ds-border)" }}>
    <Container>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(240px,1fr))", gap: "1.45rem", marginTop: "var(--ds-space-block, 2rem)" }}>
        {content.items.map((s, i) => (
          <article key={i} className="ds-card" style={{ background: "var(--ds-surface)", border: "1px solid var(--ds-border)", borderRadius: "var(--ds-radius)", padding: "1.7rem", display: "flex", flexDirection: "column", gap: "1.05rem" }}>
...
export const ProcessBadgeSteps: React.FC<{ content: ProcessContent }> = ({ content }) => (
  <section style={{ background: "var(--ds-surface)", paddingBlock: "var(--ds-section-y)", borderBottom: "1px solid var(--ds-border)" }}>
    <Container>
      <SectionHead eyebrow={content.eyebrow} heading={content.heading} />
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px,1fr))", gap: "1.45rem", marginTop: "var(--ds-space-block, 2rem)" }}>
        {content.steps.map((s, i) => (
          <div key={i} className="ds-card" style={{ background: "var(--ds-bg)", border: "1px solid var(--ds-border)", borderRadius: "var(--ds-radius)", padding: "1.8rem", display: "flex", flexDirection: "column", gap: "1.05rem" }}>
    ...
    <section style={{ background: "var(--ds-bg)", paddingBlock: "var(--ds-section-y)", borderBottom: "1px solid var(--ds-border)" }}>
      <Container>
        <div style={{ display: "grid", gridTemplateColumns: hl.length ? "minmax(0,1.5fr) minmax(0,1fr)" : "minmax(0,1fr)", gap: "2.6rem", alignItems: "start" }}>
          <div style={{ display: "flex", flexDirection: "column", gap: "1.25rem", maxWidth: "62ch" }}>
...
export const StatsDecoratorGrid: React.FC<{ content: StatsContent }> = ({ content }) => (
  <section style={{ background: "var(--ds-surface)", paddingBlock: "var(--ds-section-y)", borderBottom: "1px solid var(--ds-border)" }}>
    <Container>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(170px,1fr))", gap: "1.45rem" }}>
        {content.items.map((s, i) => (
...
export const CtaMaskedPanel: React.FC<{ content: CtaContent }> = ({ content }) => (
  <section style={{ background: "var(--ds-bg)", paddingBlock: "var(--ds-section-y)" }}>
    <Container>
      <div style={{ borderRadius: "var(--ds-radius)", background: "var(--ds-text)", padding: "3rem 2rem", textAlign: "center", display: "flex", flexDirection: "column", gap: "1.25rem", alignItems: "center" }}>
        ...
        <div style={{ marginTop: "0.5rem" }}><InvertedTone><Button variant="primary" cta>{content.button}</Button></InvertedTone></div>
      </div>
    </Container>
  </section>
);
