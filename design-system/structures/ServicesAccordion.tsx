        ...
        <div style={{ border: "1px solid var(--ds-border)", borderRadius: "var(--ds-radius)", overflow: "hidden" }}>
          {content.items.map((s, i) => (
            <div key={i} style={{ borderTop: i ? "1px solid var(--ds-border)" : "none" }}>
              <button onClick={() => setOpen(open === i ? -1 : i)} aria-expanded={open === i} aria-controls={`${uid}-p${i}`} style={{ width: "100%", textAlign: "left", background: open === i ? "var(--ds-surface)" : "var(--ds-bg)", border: "none", cursor: "pointer", padding: "1.1rem 1.4rem", display: "flex", justifyContent: "space-between", alignItems: "center", gap: "1.25rem" }}>
              ...
              {open === i && (
                <div id={`${uid}-p${i}`} role="region" style={{ padding: "0 1.4rem 1.3rem", display: "flex", flexDirection: "column", gap: "1.05rem" }}>
                  {s.image && (
