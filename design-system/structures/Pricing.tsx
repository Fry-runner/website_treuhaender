      ...
      <div style={{ display: "grid", gridTemplateColumns: `repeat(${content.tiers.length}, minmax(0,1fr))`, gap: "1.2rem", alignItems: "start" }}>
        {content.tiers.map((t, i) => (
          <div key={i} style={{
            ...
            {t.recommended && (
              <div style={{ alignSelf: "flex-start", fontFamily: "var(--ds-font-mono)", fontSize: "0.64rem", textTransform: "uppercase", letterSpacing: "0.08em", color: "var(--ds-primary-fg)", background: "var(--ds-primary)", padding: "0.2rem 0.6rem", borderRadius: "var(--ds-radius-pill)" }}>Empfohlen</div>
            )}
            ...
            <ul style={{ listStyle: "none", margin: 0, padding: 0, display: "flex", flexDirection: "column", gap: "0.6rem", flex: 1 }}>
              {t.features.map((f, j) => (
                <li key={j} style={{ display: "flex", gap: "0.5rem", fontSize: "0.9rem", color: "var(--ds-text)" }}>
                  <span style={{ color: "var(--ds-primary)", fontWeight: 700 }}>✓</span> {f}
                </li>
              ))}
            </ul>
